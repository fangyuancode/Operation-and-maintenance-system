<template>
	<el-form @submit.native.prevent class="lay-con-layer" ref="equipmentM" label-width="100px"
		:model="equipmentM"
		:rules="rules">
		<el-page-header class="form-header" @back="resetForm('equipmentM')" content="新增设备台账"></el-page-header>
		<div class="cus-from-inline">
			...
			<el-form-item label="设备分类:" prop="EquipClassifyIdTag">
				<el-select v-model="equipmentM.EquipClassifyIdTag" placeholder="请选择设备分类">
					<el-option
						v-for="item in equipClassifyArray"
						:label="item.EquipClassifyName"
						:value="item.EquipClassifyId"
						:key="item.EquipClassifyId"></el-option>
				</el-select>
			</el-form-item>
		</div>
		<div class="cus-from-inline">
			...
			<el-form-item label="工厂部门:" prop="SectionIdTag">
				<el-select v-model="equipmentM.SectionIdTag" placeholder="请选择工厂部门">
					<el-option
						...
						v-if="item.FactoryIdTag==equipmentM.FactoryIdTag"
			...
			<el-form-item label="装 置:" prop="DeviceIdTag">
				<el-select v-model="equipmentM.DeviceIdTag" placeholder="请选择装置">
					<el-option
						...
						v-if="item.FKSectionId==equipmentM.SectionIdTag"
			...
			<el-form-item label="区 域:" prop="AreaIdTag">
				<el-select v-model="equipmentM.AreaIdTag" placeholder="请选择区域">
					<el-option
						...
						v-if="item.DeviceIdTag==equipmentM.DeviceIdTag"
						:label="item.AreaName"
						:value="item.AreaId"
