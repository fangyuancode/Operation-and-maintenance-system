  ...
  <div class="lay-container">
    <div class="lay-container-pan" v-show="isMain">
      ...
      <div class="lay-container-content">
        ...
        >
          <ux-table-column title="材料类型名" field="MaterialTypeName">
            <template slot-scope="scope">
              <input class="input-edit" type="text" v-model="scope.row.MaterialTypeName" v-show="scope.row.Iseditor" />
              <span v-show="!scope.row.Iseditor">{{ scope.row.MaterialTypeName }}</span>
            </template>
          </ux-table-column>
...
export default {
  ...
  methods: {
    ...
    edit(row, scope) {
      if (row.Iseditor) {
        if (this.$verifiy.textValid(row.MaterialTypeName, 30, '类型名')) {
          return
        }
    ...
    currChange(curr) {
      this.dataTable.forEach((d) => {
        if (d.Iseditor && d.MaterialTypeId != curr.MaterialTypeId) {
          d.Iseditor = false
        }
      })
    },
